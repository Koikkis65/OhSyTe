        ...
        for event in events {
            if event.month_day() == this_day {
                ...
                if event.category.secondary == None {
                    println!("year: {} {} categories: {}", event.year(), event.description, event.category.primary);
                }
                else {
                    println!("year: {} {} categories: {}, {:#?}", event.year(), event.description, event.category.primary, event.category.secondary.as_ref().unwrap().to_string());
                }
            }
        }
        if !has_header {
