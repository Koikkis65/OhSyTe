...
impl EventProvider for SQLiteProvider {
    ...
    fn get_events(&self, events: &mut Vec<Event>) {
        let connection = match sqlite::Connection::open(self.path.clone()) {
            Ok(conn) => conn,
            Err(_) => {
                eprintln!("Unable to open SQLite database at {}", self.path.display());
                return;
            }
        };
        ...
        while let Ok(State::Row) = statement.next() {
            ...
            let category_id = statement.read::<i64, _>("category_id").unwrap();
            let category: &Category = category_map.get(&category_id).unwrap();
    ...
    fn get_events() -> Result<(), String> {
        ...
        if let Ok(sqlite::State::Row) = statement.next() {
            let expected = (
                ...
                "Unit test for SQLiteProvider".to_string(),
                1,
            );
            let actual = (
